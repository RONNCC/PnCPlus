...
with open(file_name, "rb") as f:
    ...
    data_col = "# Appts"
    #TODO: wrap the following line in a try-catch that can handle
    #the StopIteration exception
    data_index = next(i for i in xrange(0, len(headers)) 
            if headers[i] == data_col)
    for row in csvReader:
        if(row[data_index] != ""):
            departments.append(row[DEPARTMENT_COLUMN])
            visit_types.append(row[VISIT_TYPE_COLUMN])
            appts.append(row[APPTS_COLUMN])

            data.append([float(x[0:len(x)-1]) for x in row[FIRST_DATA_COLUMN:
                LAST_DATA_COLUMN + 1]])
...
for datum in data:
    plt.pie(datum,labels=headers[FIRST_DATA_COLUMN:LAST_DATA_COLUMN+1], colors=tableau20[0:LAST_DATA_COLUMN - FIRST_DATA_COLUMN + 1],autopct='%1.1f%%',shadow=True,startangle=90)
    plt.axis('equal')
    plt.show()
sys.exit()
